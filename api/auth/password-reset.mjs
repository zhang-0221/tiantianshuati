...
export default async function handler(req) {
  ...
  const limited = await rateLimit(req, 'password-reset', 5, 300);
  ...
  try {
    ...
    if (!username) return json({ ok: true }, 200, cors(req, env.ALLOWED_ORIGIN));
    ...
    if (profile.data?.email) {
      ...
      await anonymous.auth.resetPasswordForEmail(profile.data.email, {
        redirectTo: `${env.SITE_URL}?reset=1`,
      });
    }
    // Always acknowledge to avoid revealing whether an account exists.
    return json({ ok: true }, 200, cors(req, env.ALLOWED_ORIGIN));
  } catch {
